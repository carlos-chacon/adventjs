...
export default function pangram(letter) {
    ...
    const letterN = letter.toLowerCase().includes('ñ');
    const newLetter = letter.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const _letter = letterN ? newLetter + 'ñ' : newLetter;

    return alphabet.every(letter => _letter.includes(letter));
}