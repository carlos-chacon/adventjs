...
export default function sumPairs(numbers, result) {
    let primerElemento;
    let segundoElemento;
    const pasadas = numbers.length * numbers.length;
    let valores = null;

    const sumar = (numbers, result) => {
        return numbers.filter((number) => {
            if (primerElemento !== undefined) {
                segundoElemento = number;
                return primerElemento + segundoElemento === result;
            }
    ...
    for (let index = 0; index < pasadas; index++) {
        valores = sumar(numbers, result);
        console.log(valores);
        if (valores.length === 2) {
            break
        } else {
            ...
            primerElemento = undefined;
            segundoElemento = undefined;
            numbers.shift();
        }
    }
    return valores;
